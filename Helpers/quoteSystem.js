...
async function saveQuote(message, bot) {
  ...
  const saveDone = [
    "Allez hop, j'enregistre sa phrase on la ressortira plus tard !",
  ...
  if (message.mentions.has(bot.user)) {
    ...
    if (message.reference && message.reference.messageId) {
      ...
      if (discordId === botDevId || discordId === botMainId) {
        return; // Ne pas enregistrer les phrases du bot
      }
      ...
      if (!doc.exists) {
        console.warn(`${await dateFormatLog()} quoteSystem : Le profil de ${discordUsername} n'existe pas.`);
        const adminChannelId = process.env.CHANNEL_ID;
        if (!adminChannelId) {
          console.warn("CHANNEL_ID non défini dans les variables d'environnement !");
        } else {
          try {
            const channel = await bot.channels.fetch(adminChannelId);
            await channel.send(`quoteSystem : Le profil pour ${discordUsername} n'existait pas et a été créé.`);
          } catch (error) {
            console.warn("Impossible d'envoyer le message dans le salon admin :", error.message);
          }
        }
      }
      ...
      if (quoteExists.exists) {
        const randomAlreadySave = alreadySave[Math.floor(Math.random() * alreadySave.length)];
      ...
      console.log(`${await dateFormatLog()} [quoteSystem] Enregistrement : ${discordUsername} a dit "${quoteContent}" dans ${message.channel.name} le ${quoteDate}.`);
      const randomSaveDone = saveDone[Math.floor(Math.random() * saveDone.length)];
      ...
      if (mentionedUsers.size > 0) {
        ...
        if (mentionedUser.id === botDevId || mentionedUser.id === botMainId) {
          const mentionedUsersArray = mentionedUsers.array();
          if (mentionedUsersArray.length > 1) {
            mentionedUser = mentionedUsersArray[1];
          }
        }
        ...
        if (!quotesSnapshot.empty) {
          ...
          while (quotes.length > 0) {
            ...
            try {
              // On ne récupère plus le message Discord original car parfois interdit par permissions
              // On se contente de répondre avec la citation et la date
              ...
              await message.reply(`" ${quoteContent} " — ${prenom}, le ${new Date(quoteDate._seconds * 1000).toLocaleDateString()}`);
